...
# 手动配置
# =========================
CKPT_PATH = r"/data/WJH/03_SpinalKVEnhance/residual_tone_mapping/checkpoints/residualALL/best.pth"
INPUT_DIR = r"/data/WJH/03_SpinalKVEnhance/residual_tone_mapping/data/trainALL/kv"
PRED_DIR = r"/data/WJH/03_SpinalKVEnhance/residual_tone_mapping/data/trainALL/pred"
NEW_PRED_DIR = r"/data/WJH/03_SpinalKVEnhance/residual_tone_mapping/data/trainALL/NewPred"
DELTA_DIR = r"/data/WJH/03_SpinalKVEnhance/residual_tone_mapping/data/trainALL/delta"
...
FIXED_H = 768
FIXED_W = 1088
NORMALIZE_MODE = "fixed_range"   # pair_independent | input_reference | fixed_range
...
def main():
    ...
    for nii_path in tqdm(nii_files, desc="Infer"):
        if nii_path[:73][-5:] == "00316":
            ...
            # 4) 推理
            x = torch.from_numpy(inp_np_01).unsqueeze(0).unsqueeze(0).float().to(DEVICE)
            pred, delta = model(x)

            input_np = tensor_to_numpy01(x[0])
            ...
            # 5) 反归一化
            pred_raw = denormalize_output(pred_np, norm_meta).astype(np.float32)
            new_pred_raw = denormalize_output(new_pred_np, norm_meta).astype(np.float32)
